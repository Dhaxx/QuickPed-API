...
class ProdutoBase(SQLModel):
    ...
    preco: Decimal = Field(
        sa_column=Column(
            Numeric(10, 2),
            CheckConstraint("preco >= 0", name="check_preco_produto_positivo"),
        )
    )
...
class GrupoAdicionalBase(SQLModel):
    ...
    min_selecoes: int = Field(default=0)
...
class GrupoAdicional(GrupoAdicionalBase, table=True):
    ...
    estabelecimento: "Estabelecimento" = Relationship(
        back_populates="grupos_adicionais"
    )
...
class AdicionalBase(SQLModel):
    ...
    preco: Decimal = Field(
        sa_column=Column(
            Numeric(10, 2),
            CheckConstraint("preco >= 0", name="check_adicional_preco_positivo"),
        )
    )
