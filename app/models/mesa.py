...
class Mesa(MesaBase, table=True):
    __table_args__ = (
        UniqueConstraint(
            "estabelecimento_id", "numero", name="uq_mesa_estabelecimento_numero"
        ),
    )
    ...
    estabelecimento_id: int = Field(
        index=True, foreign_key="estabelecimento.id", nullable=False
    )
    ...
    criado_em: Optional[datetime] = Field(default_factory=settings.time_now)
