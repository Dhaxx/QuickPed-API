...
def create_mesa(data: MesaCreate, session = Depends(get_session), estabelecimento_id: int = Depends(get_current_estabelecimento), _=Depends(require_permission("mesas", "editar"))):
...
def get_mesas( session = Depends(get_session), estabelecimento_id: int = Depends(get_current_estabelecimento), _=Depends(require_permission("mesas", "visualizar")) ):
    return mesa_service.get(session, estabelecimento_id)

@router.put("/", response_model=MesaRead, status_code=status.HTTP_200_OK)
def update_mesa( mesa_id: int, data: MesaUpdate, session = Depends(get_session), estabelecimento_id: int = Depends(get_current_estabelecimento), _=Depends(require_permission("mesas", "editar"))):
...
def delete_mesa( mesa_id: int, session = Depends(get_session), estabelecimento_id: int = Depends(get_current_estabelecimento), _=Depends(require_permission("mesas", "editar"))):
