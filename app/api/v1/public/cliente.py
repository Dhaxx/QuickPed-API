...
from app.schemas.cliente import ClienteRead, ClienteUpdate, EnderecoCreate, EnderecoRead, EnderecoUpdate
...
def atualizar_cliente(cliente_update: ClienteUpdate, cliente: Cliente = Depends(get_current_cliente), session = Depends(get_session)):
    return cliente_service.update(session, cliente.id, cliente_update)

@router.post("/endereco", response_model=EnderecoRead, status_code=status.HTTP_201_CREATED)
def adicionar_endereco_cliente(endereco: EnderecoCreate, cliente: Cliente = Depends(get_current_cliente), session = Depends(get_session)):
...
def atualizar_endereco_cliente(endereco_id: int, endereco_update: EnderecoUpdate, cliente: Cliente = Depends(get_current_cliente), session = Depends(get_session)):
    cliente_id = cliente.id
